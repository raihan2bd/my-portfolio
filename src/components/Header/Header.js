...
const Header = () => {
  ...
  const navLinkClasses = 'border-sky-900 border p-2 text-sky-400 hover:text-white hover:bg-sky-900 block w-full cursor-pointer md:rounded';
  ...
  return (
    <header className="z-20 flex items-center justify-between p-4 h-20 fixed top-0 left-0 right-0 border-sky-900 border-b backdrop-blur md:px-6">
      ...
      <nav className={navClasses}>
        <ul className="flex gap-3 flex-col mt-6 p-4 md:flex-row md:mt-0 md:p-0">
          <li
          ...
          <li
            className={`${navLinkClasses}${activeNavItem === 'projects'? ' bg-sky-900/50 text-white': ''}`}
            onClick={() => hideNavHandler("projects")}
          >
            Projects
          </li>
          <li
            className={`${navLinkClasses}${activeNavItem === 'about'? ' bg-sky-900/50 text-white': ''}`}
            onClick={() => hideNavHandler("about")}
          >
            About
          </li>
          <li
            className={`${navLinkClasses}${activeNavItem === 'contact'? ' bg-sky-900/50 text-white': ''}`}
